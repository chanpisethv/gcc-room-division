...
// Room configuration: [roomId, capacity, type, allowedGender, allowedType]
const ROOM_CONFIG = [
...
function checkEligibility(roomId, person, rooms, forceOverride = false) {
  ...
  // If force override is enabled, only check gender/type, not capacity
  const { gender, type } = person;

  // Check if person type matches room type
  if (config.allowedType === 'both') {
    // Room 14: Female Teachers + Female Students
  ...
  // If force override is enabled, skip capacity check
